...
#' @export
calculate_moduli_multifiles2<-function(folder,Thicknesses,Strain,indenterradius,poisson_eq=0.5,poisson_inst=0.5,smfl=19,EqDataPoints = 201){
  ...
  #B.2. Normalizing the base line
  tdfn<-NormalizeBaseline(tdfn)
  #B.3. Smoothing data#
  tdfn_sm<-smooth_SR(tdfn,smfl=smfl)
  #B.4. Remove Negative peaks
  ...
  #B.5. Remove Abnormal Peaks
  #tdfn_sm<-Rmv_abnormal_peaks(tdfn_sm,S1i = SRdata[[3]][1],S2i = SRdata[[3]][2],S3i = SRdata[[3]][3])
  #tdfn<-Rmv_abnormal_peaks(tdfn,S1i = SRdata[[3]][1],S2i = SRdata[[3]][2],S3i = SRdata[[3]][3])
  #################### C.Extracting parameters######################
  Stepsindecies<-SRdata[[3]]
  S1i<-Stepsindecies[1]; S2i<-Stepsindecies[2]; S3i<-Stepsindecies[3]
  SRParameters_Sm<-Extract_SR_Parameters2(SRmergedfile=tdfn_sm,S2i,S3i, EqDataPoints=EqDataPoints)
  SRParameters<-Extract_SR_Parameters2(tdfn,S2i,S3i, EqDataPoints=EqDataPoints)
